...
def load_model(repo, model, branch='main', force_reload=False, **kwargs):
    hub_dir = Path(torch.hub.get_dir())
    repo_dir = hub_dir / f"{repo.replace('/', '_')}_{branch}"
    if repo_dir.exists() and not force_reload:
        return torch.hub.load(str(repo_dir), model, source='local', **kwargs)
    else:
        return torch.hub.load(repo, model, force_reload=force_reload, **kwargs)
