...
__global__ void patch_retr_kernel(
    torch::PackedTensorAccessor32<int64_t,1,torch::RestrictPtrTraits> index,
...
__global__ void reprojection_residuals_and_hessian(
    ...
    const torch::PackedTensorAccessor32<int64_t,2,torch::RestrictPtrTraits> ij_xself,
    ...
    const torch::PackedTensorAccessor32<int64_t,1,torch::RestrictPtrTraits> kk,
    const torch::PackedTensorAccessor32<int64_t,1,torch::RestrictPtrTraits> ku,
    ...
    torch::PackedTensorAccessor32<mtype,1,torch::RestrictPtrTraits> u, const int t0, const int ppf)
{
  ...
  GPU_1D_KERNEL_LOOP(n, ii.size(0)) {
    ...
    float Jj[6], Ji[6], Jz, r, w;
    for (int row = 0; row < 2; row++) {
        // 根据 row 来计算 r, w, Jz 和 Jj
        if (row == 0) {
            r = target[n][0] - x1;
            w = mask * weight[n][0];
        ...
        // --- 优化部分：预计算乘法因子 ---
        // 将会在循环中重复使用的乘法结果提前计算出来
        const float wr = w * r;
        const float wz = w * Jz;

        // 计算 r_total
        atomicAdd(&r_total[0], wr * r); // w * r * r -> wr * r

        // adjSE3 应该接收 float* 指针，数组名 Jj 会自动衰变(decay)为指针
        adjSE3(tij, qij, Jj, Ji);

        // --- 优化部分：预计算 w*Ji 和 w*Jj ---
        // 为更新 B 矩阵的循环做准备，减少内部的乘法次数
        float wJi[6], wJj[6];
        for (int i = 0; i < 6; ++i) {
            wJi[i] = w * Ji[i];
            wJj[i] = w * Jj[i];
        ...
        // 更新 B 矩阵 (Hessian Matrix)
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 6; j++) {
                if (ix >= 0)
                    atomicAdd(&B[6 * ix + i][6 * ix + j], wJi[i] * Ji[j]);
                if (jx >= 0)
                    atomicAdd(&B[6 * jx + i][6 * jx + j], wJj[i] * Jj[j]);
                if (ix >= 0 && jx >= 0) {
                    // 注意这里是减法
                    const float cross_term = -wJi[i] * Jj[j];
                    atomicAdd(&B[6 * ix + i][6 * jx + j], cross_term);
                    atomicAdd(&B[6 * jx + j][6 * ix + i], cross_term); // B[j][i] = B[i][j]
                }
            }
        }
        ...
        for (int i = 0; i < 6; i++) {
            if (eff_impl) {
                atomicAdd(&E_lookup[ijs][kx % ppf][i], -wz * Ji[i]);
                atomicAdd(&E_lookup[ijx][kx % ppf][i],  wz * Jj[i]);
            } else {
                if (ix >= 0)
                    atomicAdd(&E[6 * ix + i][k], -wz * Ji[i]);
                if (jx >= 0)
                    atomicAdd(&E[6 * jx + i][k],  wz * Jj[i]);
            }
        }

        // 更新 v 向量 (Gradient Vector)
        for (int i = 0; i < 6; i++) {
            if (ix >= 0)
                atomicAdd(&v[6 * ix + i], -wr * Ji[i]);
            if (jx >= 0)
                atomicAdd(&v[6 * jx + i],  wr * Jj[i]);
        }

        // 更新 C 和 u
        atomicAdd(&C[k], wz * Jz); // w * Jz * Jz -> wz * Jz
        atomicAdd(&u[k], wr * Jz); // w * r * Jz -> wr * Jz
    }
  }
}
    ...
    const int t0, const int t1, const int iterations, bool eff_impl)
{
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    reprojection_residuals_and_hessian<<<NUM_BLOCKS(ii.size(0)), NUM_THREADS>>>(
      ...
      blockE->ij_xself.packed_accessor32<int64_t,2,torch::RestrictPtrTraits>(),
      ii.packed_accessor32<int64_t,1,torch::RestrictPtrTraits>(),
      jj.packed_accessor32<int64_t,1,torch::RestrictPtrTraits>(),
      kk.packed_accessor32<int64_t,1,torch::RestrictPtrTraits>(),
      ku.packed_accessor32<int64_t,1,torch::RestrictPtrTraits>(),
