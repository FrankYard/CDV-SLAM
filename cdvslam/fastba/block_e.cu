...
#define CREATE_IDX_ACC(t, d)              \
  ...
  const auto acc_##t = cpu_##t.accessor<int64_t, d>();

typedef Eigen::Array<int64_t, -1, -1> IndexLookup;
...
{
  const int64_t n_frames = std::max(ii.max().item<int64_t>(), jj.max().item<int64_t>()) + 1;
  ...
  { // Create mapping from (frame, patch) -> index in vec
    ...
    auto patch_to_ku_acc = patch_to_ku.accessor<int64_t, 2>();
    CREATE_IDX_ACC(ku, 1)
    for (int idx = 0; idx < cpu_ku.size(0); idx++)
    {
      const int64_t k = acc_ku[idx]; // the actual uniq value. idx is the row in Q where it was found
  ...
  { // Create mapping from (i,j) -> E_lookup
    ...
    {
      const int64_t i = acc_ii[idx];
      const int64_t j = acc_jj[idx];
      const int64_t ijx = acc_ij_xself[0][idx];
      const int64_t ijs = acc_ij_xself[1][idx];
    ...
    const int64_t E = cpu_ii.size(0);
    std::vector<std::unordered_set<int64_t>> edge_lookup(n_frames);
    for (int x = 0; x < E; x++)
    {
      const int64_t i = acc_ii[x];
      const int64_t j = acc_jj[x];
    ...
    auto index_tensor_acc = index_tensor.accessor<int64_t, 2>();
    // std::cout << "#W" << std::endl;
    ...
    for (int i = 0; i < n_frames; i++)
    {
      ...
      for (const int64_t &j1 : connected_frames)
      {
        for (const int64_t &j2 : connected_frames)
        {
...

  {
    ...
    const int64_t count = ij_uniq.size(0);
    block_index_tensor = torch::empty({count, 2}, torch::kInt64);
    auto index_tensor_acc = block_index_tensor.accessor<int64_t, 2>();
    for (int idx = 0; idx < count; idx++)
    {
      const int64_t ij = acc_ij_uniq[idx];
      const int64_t i = ij / n_frames;
      const int64_t j = ij % n_frames;
...
__global__ void EEt_kernel(
    ...
    const torch::PackedTensorAccessor32<int64_t, 2, torch::RestrictPtrTraits> index_tensor,
    const torch::PackedTensorAccessor32<int64_t, 2, torch::RestrictPtrTraits> patch_to_ku, const int t0, const int ppf)
{
...
torch::Tensor EfficentE::computeEQEt(const int N, const torch::Tensor &Q) const
{
  ...
  EEt_kernel<<<NUM_BLOCKS(index_tensor.size(0) * ppf), NUM_THREADS>>>(
      ...
      index_tensor.packed_accessor32<int64_t, 2, torch::RestrictPtrTraits>(),
      patch_to_ku.packed_accessor32<int64_t, 2, torch::RestrictPtrTraits>(),
      t0, ppf);
...
torch::Tensor EfficentE::computeEv(const int N, const torch::Tensor &vec) const
{
  ...
  Ev_kernel<<<NUM_BLOCKS(E_lookup.size(0) * ppf), NUM_THREADS>>>(
      ...
      block_index_tensor.packed_accessor32<int64_t, 2, torch::RestrictPtrTraits>(),
      patch_to_ku.packed_accessor32<int64_t, 2, torch::RestrictPtrTraits>(),
      t0, ppf);
