...
class Update(nn.Module):
    ...
    def forward(self, net, inp, corr, flow, ii, jj, kk, posenc_cache=None):
        """ update operator """
...
class Patchifier(nn.Module):
    ...
    def forward(self, images, patches_per_image=80, disps=None, centroid_sel_strat='RANDOM', return_color=False,
        ...
        coords = torch.stack([x, y], dim=-1).float()
        imap = altcorr.patchify(imap[0], coords, 0).view(b, -1, DIM, 1, 1) # TODO: Only correct when b==1
        gmap = altcorr.patchify(fmap[0], coords, P//2).view(b, -1, 128, P, P) # b, patches_per_image, ...
        ...
        index = index.repeat(1, patches_per_image).reshape(-1) # n, patches_per_image
        scores = None

        if return_color:
            return fmap, gmap, imap, patches, index, clr, scores
...
class VONet(nn.Module):
    ...
    def forward(self, images, poses, disps, intrinsics, M=1024, STEPS=12, P=1, structure_only=False, rescale=False):
        ...
        fmap, gmap, imap, patches, ix, scores = self.patchify(images, disps=disps) # patches:(b, -1, 3, P, P)
