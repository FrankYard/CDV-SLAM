...
def run(cfg, network, scene_dir, sequence, stride=1, viz=False, show_img=False):
    ...
    for step in range(sys.maxsize):
        ...
        with Timer("SLAM", enabled=False):
            out = slam(t, images, intrinsics)
        # print(step, t , slam.poses[:, :, :3].max(), out)
...
if __name__ == '__main__':
    ...
    for scene in tum_scenes:
        ...
        for trial_num in range(args.trials):
            traj_est, timestamps = run(cfg, net, scene_dir, scene, args.stride, args.viz, args.show_img)
    ...
    import os
    file_path = os.path.join('results', f"tum_{args.expname}.txt")
